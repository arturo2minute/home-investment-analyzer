...
export default function BuyRent() {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/property/${id}/short_term`);
        ...
        <div className="deal-inputs-wrapper p-5 mx-auto space-y-8">
          ...
          <div className="bg-white p-6 rounded-xl shadow border space-y-8">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                {["yearly_taxes", "monthly_insurance", "cleaning", "internet", "hoa_fees", "gas", "electricity", "watersewer", "garbage", "other"].map((name) => (
                  <div key={name}>
                    <label className="block text-sm font-medium text-dark-gray mb-1">
                      {name.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase())}
                    </label>
