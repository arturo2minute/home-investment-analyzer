...
export default function BuyRent() {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      try {
        const res = await axios.get(`http://localhost:8000/property/${id}/brrrr`);
                ...
                {!inputs.cash && (
                  <>
                    {["refinance_loan_amount", "refinance_interest_rate", "refinance_lender_charges", "refinance_loan_fees_wrapped", "refinance_pmi", "refinance_years_amortized"].map((name) => (
                      <div key={name}>
                        <label className="block text-sm font-medium text-dark-gray mb-1">
                          {name === "refinance_pmi" ? "PMI" :
                           name.replace(/_/g, " ").replace(/refinance/gi, "").replace(/\b\w/g, l => l.toUpperCase()).trim()}
                        </label>
                        <input
                          type={name === "refinance_loan_fees_wrapped" ? "checkbox" : "number"}
                          name={name}
                          value={name === "refinance_loan_fees_wrapped" ? undefined : inputs[name]}
                          checked={name === "refinance_loan_fees_wrapped" ? inputs[name] : undefined}
                          onChange={handleInputChange}
                          className={name === "refinance_loan_fees_wrapped" ? "w-4 h-4" : "w-full border rounded px-3 py-2 text-sm"}
                          placeholder={name === "refinance_loan_fees_wrapped" ? undefined : "0"}/>
                      </div>
                    ))}
                  </>
                )}
